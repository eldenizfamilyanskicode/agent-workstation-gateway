...
This document describes the implemented shared execution-policy/lifecycle core and native Windows/Linux v0.1 boundaries.
...
The envelope contract does not authenticate a caller by itself. On Windows, [`internal/platform/windows/brokeripc`](../internal/platform/windows/brokeripc) supplies the fixed named-pipe boundary: an exact protected DACL, first-instance/local-only modes, a fixed authentication preface, and exact impersonated TokenUser SID authorization before it exposes framed envelope bytes. Linux uses a root-owned Unix socket with exact owner/group/mode and `SO_PEERCRED` control-UID verification. See [ADR 0010](adr/0010-windows-authenticated-named-pipe.md) and [ADR 0021](adr/0021-linux-systemd-uid-and-unix-socket-boundary.md).
...
This resolver is not an ACL sandbox and does not eliminate every replacement race by itself. The Windows launcher repeats handle/final-path checks near launch and the Linux resolver uses descriptor-relative no-follow traversal; protected ACLs deny unrelated data and native artifact collectors independently enforce containment under execution authority.
...
Explicit `start`, `status`, `logs`, and `stop` requests reuse that exact authorized launch plan. A bounded broker-memory registry owns at most 32 `session_id`/`process_id` entries, retains only bounded output, rejects cross-session/path lookup, expires each tree at its accepted lifetime, and synchronously reaps it on stop or broker shutdown. Ordinary execution still cannot detach. [ADR 0020](adr/0020-bounded-background-process-lifecycle.md) records the lifecycle and evidence requirements.
...
Local Windows tests exercise Job Object tree termination, DPAPI round-trip/tamper rejection, ACL policy and denial, batch-logon behavior, named-pipe authentication, service configuration, response publication, and installer rollback. The isolated Windows acceptance matrix additionally exercised elevated provisioning, LocalSystem-to-execution-account launch, installed service/runner lifecycle, credential/filesystem denials, private transport, artifacts, doctor, uninstall, and reinstall.

## Native enforcement and evidence

v0.1 implements the service, peer-authentication, identity-transition, filesystem, process-lifecycle, output, artifact, and private-control boundaries on both supported platforms. Windows uses SCM/LocalSystem, exact named-pipe caller SID checks, restricted account tokens, ACLs, and Job Objects. Linux uses hardened systemd units, exact Unix peer UID checks, a fixed UID/GID transition with cleared capabilities and no-new-privileges, POSIX ACLs, and process groups.

The Windows x64 boundary has inspected isolated-host E2E evidence. The Linux x64 boundary has inspected WSL2/systemd/private-control E2E evidence plus Ubuntu, Debian, and Fedora container compatibility checks. WSL2, containers, and cross-compilation do not prove every bare-metal distribution or host policy. Release/support scope follows [ADR 0022](adr/0022-v0.1-release-and-upgrade.md).

## Security consequences

The implemented shared layer narrows what the broker may accept and makes unsafe authority fields structurally unavailable. It is defense against confused-deputy behavior, not a substitute for OS isolation.

AWG narrows a trusted agent's workstation authority. It does not isolate mutually hostile tenants or make malicious code safe, and operators remain responsible for the roots and optional tools they grant.
