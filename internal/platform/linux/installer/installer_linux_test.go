...
func TestExpectedUnitsRetainSecurityBoundary(t *testing.T) {
	...
	for _, expected := range []string{"User=root", "NoNewPrivileges=true", "PrivateTmp=false", "ProtectSystem=strict", "CapabilityBoundingSet=CAP_SETUID CAP_SETGID CAP_KILL CAP_CHOWN", "ReadWritePaths=/run/agent-workstation-gateway"} {
		if !strings.Contains(broker, expected) {
			t.Fatalf("broker unit is missing %q", expected)
		}
	}
	for _, expected := range []string{"User=awg-control", "Group=awg-control", "NoNewPrivileges=true", "ProtectSystem=strict", "RestrictSUIDSGID=false", "WorkingDirectory=/opt/agent-workstation-gateway-runner"} {
