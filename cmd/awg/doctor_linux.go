...
func runDoctor(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer, version, sourceSHA string) int {
	...
	defer client.Close()
	if client.VerifyExclusivePrivate(ctx) != nil {
		fmt.Fprintln(stderr, "private control repository is invalid")
		return 1
	}
	if client.VerifyRunner(ctx, installation.Metadata.RunnerName) != nil {
		fmt.Fprintln(stderr, "registered runner identity is invalid")
		return 1
	}
	for _, file := range installation.Metadata.ControlFiles {
		if client.VerifyOwnedControlFile(ctx, file.Path, file.SHA256) != nil {
